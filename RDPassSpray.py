...
# It's a cool and useful script but there's a couple things I wanted to change like colours and the logging
# Not gonna lie, it could definitely use some work.
...
def attempts(users, passes, targets, domain, output_file_name, hostnames_stripped, sleep_time,
    ...
    try:
        LOGGER.info(
            "\033[0;37;40m[*] Started running at: %s" % datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S'))
        ...
        LOGGER.info("\033[0;37;40m[*] Overall compromised accounts: %s" % working_creds_counter)
        LOGGER.info(
            "\033[0;37;40m[*] Finished running at: %s" % datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S'))
        subprocess.call("hostnamectl set-hostname '%s'" % orighostname, shell=True)
...
def main():
    ...
    LOGGER.info("\033[0;37;40mTotal number of users to test: " + str(total_accounts))
    LOGGER.info("\033[0;37;40mTotal number of password to test: " + str(total_passwords))
    LOGGER.info("\033[0;37;40mTotal number of attempts: " + str(total_attempts))
